...
public interface ImgLogDao {
    ...
    @Delete("delete from img_log where 1=1")
    int delAllImg();
     ...
     * @return 影响行数
     */
    int batchDelImg(List<Integer> ids);

    @Select("select * from img_log order by create_date desc limit #{offset},#{limit}")
    List<ImgLog> queryAdminList(@Param("offset") Integer offset, @Param("limit") Integer limit);
